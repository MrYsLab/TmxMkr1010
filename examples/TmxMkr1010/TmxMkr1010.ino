...
uint16_t IP_PORT = 31335;

WiFiServer wifiServer(IP_PORT);
...
int analog_read_pins[] = {A0, A1, A2, A3, A4, A5, A6};
...
void sonar_new() {
    ...
    sonars[sonars_index].usonic = new Ultrasonic((uint8_t)command_buffer[0], (uint8_t)command_buffer[1], 80000UL);
...
void read_blocking_spi() {
    ...
    // report_message[0] = length of message including this element
    // report_message[1] = SPI_REPORT
    // report_message[2] = register used for the read
    // report_message[3] = number of bytes returned
    // report_message[4..] = data read
